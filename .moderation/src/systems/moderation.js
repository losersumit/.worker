...
export async function moderateMessage(messageContent) {
  try {
    ...
    if (
      !shouldFlag &&
      config.moderation.strictMode &&
      moderationResult.confidence &&
      moderationResult.confidence > 0.4
    ) {
      shouldFlag = true;
    ...
    if (config.logging.logAllDecisions && config.logging.consoleLog) {
      console.log(
        `Moderation decision for message: ${messageContent.substring(0, 30)}...`,
      );
      console.log(
        `Decision: ${shouldFlag ? "FLAGGED" : "ALLOWED"}, Reason: ${moderationResult.reason || "N/A"}`,
      );
    }
...
export async function moderateImage(imageUrl) {
  ...
  try {
    const prompt = `
      ...
      If it is related to Mr Beast twitter screenshot flag it. It is a scam.
    ...
    const responseText = completion?.choices?.[0]?.message?.content || "";
    console.log(
      `[Moderation] Raw response text: ${responseText.substring(0, 100)}...`,
    );
    ...
    try {
      ...
      console.error(
        `[Moderation] Failed to parse JSON response: ${responseText}`,
      );
      throw e;
    }
    ...
    if (config.logging.logAllDecisions && config.logging.consoleLog) {
      console.log(
        `Moderation decision for image: ${imageUrl.substring(0, 50)}...`,
      );
      console.log(
        `Decision: ${shouldFlag ? "FLAGGED" : "ALLOWED"}, Reason: ${moderationResult.reason || "N/A"}`,
      );
