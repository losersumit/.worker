...
`;
    ...
    return aiResponse?.choices?.[0]?.message?.content?.trim().toUpperCase() || 'NO';
}
...
`;
    ...
    const answer = aiResponse?.choices?.[0]?.message?.content?.trim().toUpperCase() || 'INVALID';
...
export async function recoverCountingState(channel) {
    ...
    while (true) {
        ...
        for (const message of messages.values()) {
            if (message.author?.bot && message.content.includes(COUNTING_RESET_TEXT)) {
                return { currentCount: 0, lastUserId: null, sourceMessage: message, reason: 'reset' };
            }
