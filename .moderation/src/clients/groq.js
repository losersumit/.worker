...
export const api_key_two =
  process.env.GROQ_API_KEY_TWO || process.env.GROQ_API_KEY || "";
...
export async function groqChatCompletion(payload) {
  ...
  try {
    console.log(
      `[GroqClient] Attempting request with key index ${activeKeyIndex}...`,
    );
    const result = await postWithKey(payload, keys[activeKeyIndex]);
    console.log(
      `[GroqClient] Request successful with key index ${activeKeyIndex}.`,
    );
    return result;
  } catch (err) {
    console.error(
      `[GroqClient] Request failed with key index ${activeKeyIndex}.`,
    );
    const status = err?.response?.status;
    const errorData = err?.response?.data?.error;
    console.error(
      `[GroqClient] Error details: Status=${status}, Code=${errorData?.code}, Message=${errorData?.message}`,
    );
    ...
    if (
      ...
      (errorData?.code === "model_not_found" ||
        ...
        errorData?.message?.includes("not exist"))
    ) {
      ...
      if (
        payload.model === config.ai.visionModel &&
        config.ai.fallbackVisionModel
      ) {
        console.warn(
          `[GroqClient] Switching to fallback vision model: ${config.ai.fallbackVisionModel}`,
        );
        ...
        try {
          console.log(
            `[GroqClient] Retrying with fallback model and key index ${activeKeyIndex}...`,
          );
          const result = await postWithKey(payload, keys[activeKeyIndex]);
        ...
        console.warn(
          "[GroqClient] No fallback model configured or already using fallback.",
        );
      }
    }
    ...
    if (!isQuotaOrTokenExhaustion(err)) {
      console.error(
        "[GroqClient] Error is NOT quota/token related (and not 401/403). Re-throwing.",
      );
      throw err;
    }
    ...
    console.warn(
      `[GroqClient] Limit/Auth issue on key ${activeKeyIndex}. Starting failover sequence...`,
    );
    ...
    while (nextIndex !== startIndex) {
      ...
      try {
        console.log(
          `[GroqClient] Retrying with key index ${activeKeyIndex}...`,
        );
        const result = await postWithKey(payload, keys[activeKeyIndex]);
        console.log(
          `[GroqClient] Retry successful with key index ${activeKeyIndex}.`,
        );
        return result;
      } catch (err2) {
        console.error(
          `[GroqClient] Retry failed with key index ${activeKeyIndex}.`,
        );
        const status2 = err2?.response?.status;
        const errorData2 = err2?.response?.data?.error;
        console.error(
          `[GroqClient] Error details: Status=${status2}, Code=${errorData2?.code}, Message=${errorData2?.message}`,
        );
        ...
        if (!isQuotaOrTokenExhaustion(err2)) {
          console.error(
            "[GroqClient] Failover error is NOT quota/token related. Re-throwing.",
          );
          throw err2;
        }
