...
if (!process.env.GROQ_API_KEY_ONE && !process.env.GROQ_API_KEY) {
  throw new Error(
    "❌ Missing Groq key: set GROQ_API_KEY_ONE (or GROQ_API_KEY for backward compatibility)",
  );
}
...
client.once("ready", async () => {
  ...
  syncDisplayNames(client, supabase).catch((err) =>
    console.error("[DISPLAYNAME-SYNC] Startup sync failed:", err),
  );
  ...
  if (process.env.GUILD_INCOME_CHANNEL_ID) {
    ...
    setupGuildIncomeListener(
      client,
      supabase,
      process.env.GUILD_ID,
      process.env.GUILD_INCOME_CHANNEL_ID,
    );
  }
  ...
  schedule.scheduleJob(rule, async () => {
    console.log(
      `⏰ Running scheduled daily tax at 12:00 AM (London Time) - ${new Date().toISOString()}`,
    );
  ...
  schedule.scheduleJob("0 4 * * *", async () => {
    console.log(
      `⏰ [MEMORY-CLEANUP] Running daily memory consolidation - ${new Date().toISOString()}`,
    );
  ...
  schedule.scheduleJob("0 1 * * *", async () => {
    console.log(
      `⏰ [INACTIVITY] Running daily inactivity scan - ${new Date().toISOString()}`,
    );
    await runInactivityScan(client, supabase);
  });

  // Run inactivity scan immediately on startup
  runInactivityScan(client, supabase).catch((err) =>
    console.error("[INACTIVITY] Startup scan failed:", err),
  );

  // Refresh Discord CDN URLs every hour (well within Discord's 24h expiry window)
  schedule.scheduleJob("0 * * * *", async () => {
    console.log(
      `⏰ [URL-REFRESH] Running hourly CDN URL refresh - ${new Date().toISOString()}`,
    );
    await refreshDiscordUrls(supabase, client);
  });

  // Run immediately on startup so the website has fresh URLs right away
  refreshDiscordUrls(supabase, client).catch((err) =>
    console.error("[URL-REFRESH] Startup run failed:", err),
  );

  // Refresh Permanent Slot Machines
  import("./.economy/utils/postSlots.js")
    .then(({ postPermanentSlots }) => {
      postPermanentSlots(client).catch((err) =>
        console.error("[SLOTS] Startup refresh failed:", err),
      );
    })
    .catch((err) => console.error("Failed to load postSlots script:", err));

  // Refresh Permanent Roulette Tables
  import("./.economy/utils/postRouletteTables.js")
    .then(({ postPermanentRouletteTables }) => {
      postPermanentRouletteTables(client).catch((err) =>
        console.error("[ROULETTE] Startup refresh failed:", err),
      );
    })
    .catch((err) =>
      console.error("Failed to load postRouletteTables script:", err),
    );
  ...
  schedule.scheduleJob("30 0 * * *", async () => {
    console.log(
      `⏰ [AVATAR-REFRESH] Running daily profile picture refresh - ${new Date().toISOString()}`,
    );
    await refreshProfilePictures(supabase, client);
  });

  // Run profile picture refresh immediately on startup
  refreshProfilePictures(supabase, client).catch((err) =>
    console.error("[AVATAR-REFRESH] Startup run failed:", err),
  );

  // ── Weekly Awards (JOTW + DOTW) every Sunday 9 PM London time ──
  schedule.scheduleJob(
    { rule: "0 21 * * 0", tz: "Europe/London" },
    async () => {
      console.log(
        `⏰ [AWARDS] Running weekly awards - ${new Date().toISOString()}`,
      );
      await runWeeklyAwards(client, supabase).catch((err) =>
  ...
  import("./.moderation/src/systems/uvsStatusMonitor.js")
    .then(({ setupUvsRunsListener, updateUvsEmbed }) => {
  ...
  setInterval(async () => {
    console.log(
      `⏰ [AUTO-RESTART] Scheduled 6-hour automatic restart triggered. Exiting process...`,
    );
      ...
      try {
        ...
        if (logChannel) {
          await logChannel.send(
            "⏰ **[SYSTEM]** Scheduled 6-hour automatic container restart initiated.",
          );
        }
      } catch (err) {
        console.error(
          "[AUTO-RESTART-LOG] Failed to send log message:",
          err.message,
        );
      }
    }
